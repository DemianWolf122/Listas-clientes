/**
 * Generador del feed iCal (.ics) de la agenda de Atrio, compartido por las
 * rutas /api/ics (query) y /api/ics/[feed] (con extensión .ics, que es lo que
 * mejor validan Apple Calendar y compañía).
 *
 * Los horarios de tareas se guardan como hora de pared de Buenos Aires (UTC-3,
 * sin horario de verano), así que se convierten a UTC para el feed.
 */
...
/** Construye el .ics completo. `assignee` (UUID) filtra las tareas de esa persona. */
export async function buildIcs(assignee: string | null): Promise<string | null> {
...
/** Respuesta HTTP estándar para un feed .ics. */
